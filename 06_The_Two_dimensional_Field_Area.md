 ...
 * 화면의 물체를 모르는 경우, 우리는 그것이 화면의 영역을 얼마나 차지하는지로 그 크기를 판단하는 경향이 있다. 스크린의 영역을 많이 차지할수록 더 크게 느껴진다.     
 ...
 > >  #### 크기(Scale)
  * 우리는 동일한 샷에 나타나는 다른 물체들을 보면, 크기에 대해 지속적으로 판단하고 계층적으로 정렬하려고 한다. 스케일은 영화 화면이든 모바일 화면이든 일정하게 유지되는 것이 편리한데, 규모를 설정하기 위해선 무엇이 크고 작은지 알려주는 규범이나 단서가 있어야 한다. 이러한 상황적 정보가 없다면, 물체를 비교하더라도 실제 크기에 대해서 아무것도 알 수 없다.   
